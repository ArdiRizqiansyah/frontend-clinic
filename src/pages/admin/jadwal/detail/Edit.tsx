...
function Create() {
  ...
  const dataLoader: { jadwal?: any, jadwal_detail?: any } = useLoaderData() ?? {};
  ...
  useEffect(() => {
    setJadwalDetail(dataLoader.jadwal_detail);
    setHari(dataLoader.jadwal_detail.hari);
    setJamMulai(dataLoader.jadwal_detail.jam_mulai);
    setJamSelesai(dataLoader.jadwal_detail.jam_selesai);
  }, [dataLoader]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    await axios
      .put("http://localhost:8000/api/admin/jadwal-detail/"+ jadwalDetail.id, formData, config)
      .then((response) => {
    ...
    <Admin>
      <div className="flex justify-between items-center mb-5">
        <h4 className="capitalize">
          Edit Data Detail Jadwal Dokter {dataLoader.jadwal.dokter.nama}
        </h4>
        <Link
          to={`/admin/jadwal/${params.id}/detail`}
...
export async function loader({ params }: any) {
  const jadwal = await axios
    ...
    const jadwal_detail = await axios
    .get(`http://localhost:8000/api/admin/jadwal-detail/${params.jadwal_detail_id}`, config)
    .then((response) => {
        return response.data.data;
    });
